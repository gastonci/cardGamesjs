...
function _cardDEBUGGER(x,y){if(_cardDEBUGGERlevel >= y){console.log(x);}}
...
var Deck = function()
{
	...
	this.image = {src:null,width:null,height:null},
	this.display = null;
	...
	function addCss(x){
		...
		if(!cssRules[0] || (cssRules[0].selectorText != cardContainerCSS)){addRule('.cardContainer','border:solid black 1px; position:absolute; overflow:hidden;',0)}
		while(cssIndex < cssRules.length){
			if(cssRules[cssIndex].selectorText == x){return false;}
			cssIndex++;
		}
		addRule(x,'',1);
		return true;
	...
	if(arg){
		...
		else{console.debug('deck() - argumento invalido: '+arg);}
	}
	else{
